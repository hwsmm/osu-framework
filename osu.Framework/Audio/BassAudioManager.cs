...
namespace osu.Framework.Audio
{
    public class BassAudioManager : AudioManager
    {
        ...
        public BassAudioManager(AudioThread audioThread, ResourceStore<byte[]> trackStore, ResourceStore<byte[]> sampleStore, [CanBeNull] FrameworkConfigManager config)
            : base(audioThread, trackStore, sampleStore, config)
        {
            GlobalMixerHandle.ValueChanged += handle => ((Bindable<bool>)UsingGlobalMixer).Value = handle.NewValue.HasValue;
        ...
        /// <summary>
        /// (Re-)Initialises BASS for the current <see cref="FrameworkSetting.AudioDevice"/>.
        ...
        protected override void InitCurrentDevice()
        {
            ...
            if ((DeviceNames.Count > 0 || RuntimeInfo.IsMobile) && trySetDevice(bass_default_device)) return;
        ...
        protected virtual bool InitBass(int device)
        {
            ...
            bool attemptInit()
            {
                bool innerSuccess = CurrentAudioThread.InitDevice(device, UseExperimentalWasapi.Value);
